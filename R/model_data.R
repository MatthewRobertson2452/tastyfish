...
#' the k paramater, a vector for all presence/absence data (pa), and an index vector identifying which functional response data will be associated with (t2dex)
#' @export
#'
#' @examples model_data(fishy_dat=fishy_dat, k=0.95, chi=0.45, id=c(0,1,1), n=c(length(df$pa),length(df1$pa),length(df2$pa)))
model_data<-function(fishy_dat, id, n){
  tmb.data<-list(
